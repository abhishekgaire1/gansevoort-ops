-- Fixes the LAST residual race in post_purchase_document_inventory,
-- caught by the concurrent-posting integration test under full-suite load
-- (after 20260811100066 fixed the two-statement snapshot race).
--
-- Sequence: caller B passes the single-snapshot eligibility check while
-- winner A is still uncommitted, inserts its posting HEADER, and only
-- then -- inside the per-location/per-line loops, whose statements each
-- take fresh READ COMMITTED snapshots -- sees every candidate line as
-- already posted by the now-committed A. B's loops insert nothing, so the
-- unique receipt_line_id backstop never fires, and B fell through to
-- report POSTED having posted zero lines, leaving a phantom empty posting
-- header behind. Inventory itself was never doubled (the unique backstop
-- still guarantees that); the defect was a lying status + an empty
-- posting record.
--
-- Fix: a poster whose loops posted ZERO lines forces itself down the
-- existing unique_violation convergence path (rolling back its own empty
-- header, reporting the winner's posting as ALREADY_POSTED). Body-only
-- replace of 20260811100066's function; identical otherwise.
