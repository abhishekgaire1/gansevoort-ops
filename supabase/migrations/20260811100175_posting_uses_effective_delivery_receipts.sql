-- Posting quantity now consumes the ONE authoritative effective-delivery
-- definition (20260811100173): every receipt-line selection in
-- post_purchase_document_inventory is filtered to the CONTRIBUTING receipts
-- (supersession + delivery_event_id + a valid resolution's duplicate
-- exclusions). This removes the last "sum every effective receipt row"
-- disagreement: a resolved-duplicate document posts only the retained
-- delivery's quantity, genuine distinct deliveries still sum, and an
-- unresolved ambiguous document is still refused by the GA080 header guard
-- before this ever runs.
--
-- Reproduced verbatim from 20260811100132 with ONLY the contributing-receipts
-- filter appended to each receipt_lines join. Grants are preserved by
-- create-or-replace.
