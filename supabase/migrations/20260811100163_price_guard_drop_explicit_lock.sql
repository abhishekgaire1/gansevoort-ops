-- Remove the explicit `for no key update` document lock from the price
-- guard. The guard recomputes the current normalized price from the current
-- database state at posting time, so a stale acknowledgment is already
-- detected by value mismatch -- the extra row lock added no safety but
-- serialized concurrent posts of the same document, disturbing the existing
-- concurrent-posting idempotency behavior. The posting function's own
-- existing locking still governs concurrency.
