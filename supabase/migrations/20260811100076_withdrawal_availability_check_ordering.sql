-- Milestone 2A.5 fix-forward -- 20260811100073/100074 already applied,
-- so this is a targeted create-or-replace (same signature). Corrects an
-- ordering bug: the availability check previously ran BEFORE the
-- movement line insert, using coalesce(p_measured_base_quantity,
-- p_entered_quantity) as an APPROXIMATION of the normalized quantity --
-- wrong for a packaging/measured-item withdrawal, and worse, it could
-- reject with "insufficient inventory" (GA022) for a request that was
-- actually invalid for a completely different reason (wrong entry unit),
-- masking the real error and breaking the existing wrong-unit-rejection
-- test coverage. Moved to run immediately AFTER the movement line
-- insert, comparing against the trigger's own AUTHORITATIVE
-- normalized_base_quantity -- unit/measurement validation now always
-- takes precedence, and a raised GA022 still rolls back the movement
-- header and line inserted just above it (one function call, one
-- transaction), so "no partial movement ever written" is unchanged.
