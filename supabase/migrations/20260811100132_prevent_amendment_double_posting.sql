-- Fix: an amendment could double-post the same physical delivery into
-- inventory.
--
-- post_purchase_document_inventory (20260811100064/100123) scopes every
-- check purely by p_purchase_document_id -- it has no awareness that a
-- purchase_document can be one revision among several sharing the same
-- revision_group_id (20260811100029/100033, initiate_purchase_document_
-- amendment). Confirmed by a real test scenario: verify and post an
-- original revision (10 PIECE posted), reopen it as an amendment,
-- re-confirm the item, re-record the SAME physical delivery (10 PIECE) on
-- the amendment's own (brand-new) receipt, verify the amendment, and post
-- it -- the balance became 20, not 10. Nothing in the schema ties the
-- amendment's receipt back to the original's as "the same delivery," so
-- posting the amendment's receipt is indistinguishable, to this function,
-- from a genuinely new one.
--
-- THE FIX: body-only replace, adding ONE new guard right after the
-- existing VERIFIED-status check -- a purchase_document may not post
-- inventory if ANY OTHER revision in its own revision_group_id has
-- already posted. This is deliberately conservative: an amendment
-- lineage may post inventory AT MOST ONCE, on whichever revision gets
-- there first (usually the original; occasionally the amendment itself,
-- when the original was verified but never posted before being
-- reopened). A genuinely NEW additional delivery for an already-VERIFIED,
-- already-posted document has its own dedicated, already-safe path
-- (recordReceiptRpc's additional-delivery flow against that SAME
-- purchase_document_id, never a new revision) and is completely
-- unaffected by this check. Every other statement below is byte-identical
-- to 20260811100123's definition.
